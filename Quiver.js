...
// --- UI ---
// Main import button - prominent with icon
...
// Also allow renaming when pressing Enter in the input field
renamerInput.onReturnPressed = function() {
    renameSelectedLayers();
};

// Test that handlers were assigned
...
function createText(node, parentId, vb) {
    ...
    if (letterSpacingRaw && ('' + letterSpacingRaw).toLowerCase() !== 'normal') {
        var lsNum = parseFloat(('' + letterSpacingRaw).replace('px',''));
